...
/datum/antagonist/bloodsucker/proc/AssignStarterPowersAndStats()
	...
	if (ishuman(owner.current))
		...
		S.punchdamage += 2      //highest possible punch damage	 9
		// Clown
		if(istype(H) && owner.assigned_role == "Clown")
			...
			to_chat(H, "As a vampiric clown, you are no longer a danger to yourself. Your nature is subdued. Poggers.")

	// Physiology
	CheckVampOrgans() // Heart, Eyes

	// Language
	owner.current.grant_language(/datum/language/vampiric, TRUE, TRUE, LANGUAGE_ATOM)
...
/datum/antagonist/bloodsucker/proc/update_bloodsucker_icons_added(datum/mind/m)
	...
	owner.current.hud_list[ANTAG_HUD].icon = image('austation/icons/Fulpicons/fulphud.dmi', owner.current, "bloodsucker")	// FULP ADDITION! Check prepare_huds in mob.dm to see why.
...
/obj/screen/bloodsucker/blood_counter		// NOTE: Look up /obj/screen/devil/soul_counter  in _onclick / hud / human.dm
	icon = 'austation/icons/Fulpicons/fulpicons.dmi'//'icons/mob/screen_gen.dmi'
	name = "Blood Consumed"
