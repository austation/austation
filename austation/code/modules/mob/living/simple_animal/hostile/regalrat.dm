/mob/living/simple_animal/hostile/regalrat
	...
	icon = 'austation/icons/mob/regalrat.dmi'
	icon_state = "regalrat"
...
/datum/action/cooldown/coffer
	...
	icon_icon = 'austation/icons/mob/actions/actions.dmi'
...
/datum/action/cooldown/coffer/Trigger()
	...
	if(prob(0.001)) //1 in a hundred thousand to roll the 7 sided die.
		switch(memeloot)
			...
			if(2)
				new /obj/item/reagent_containers/food/snacks/store/bread/recycled/antimatter(T)
				to_chat(owner, "<span class='notice'>Oh shit! It's Antibread! EVERY RAT FOR THEIR SELVES!</span>")
				message_admins("A rat has dug up a antimatter loaf at [ADMIN_VERBOSEJMP(T)]")
				log_game("A rat has dug up a antimatter loaf at [AREACOORD(T)]")
			if(3)
				new /obj/item/toy/spinningtoy(T)
				to_chat(owner, "<span class='notice'>Oh shit! Its loose!</span>")
			if(4)
				new /obj/item/fakeartefact(T)
				to_chat(owner, "<span class='notice'>Wow some kind of legendary artefact!</span>")
			if(5)
				new /mob/living/simple_animal/bot/secbot/deathsky/toy(T)
				to_chat(owner, "<span class='notice'>What kind of machine is this?</span>")
			if(6)
