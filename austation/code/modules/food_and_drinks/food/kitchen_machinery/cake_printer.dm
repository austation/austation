	...
	if(istype(I, /obj/item/reagent_containers))
		if(I.reagents.has_reagent(/datum/reagent/consumable/synthetic_cake_batter))
			...
			if(fuel == max_fuel)
				...
				return ..()
			if((10 * (reagent_source.amount_per_transfer_from_this) > max_fuel))
				to_chat(user, "<span class='warning'>Too much reagent per transfer, the tank can't hold it all!</span>")
				return ..()
			fuel += (10 * (reagent_source.amount_per_transfer_from_this))
			to_chat(user, "<span class='notice'>You pour the cake batter in [src].</span>")
			I.reagents.remove_reagent(/datum/reagent/consumable/synthetic_cake_batter, reagent_source.amount_per_transfer_from_this)
			if(fuel > max_fuel)
				fuel = max_fuel
			return ..()
