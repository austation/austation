...
/obj/structure/disposalpipe/segment/coilgun
	...
	var/const/safe_corner_velocity = 30 // maximum projectile velocity on corner pipes before they'll start breaking the pipes
...
/obj/structure/disposalpipe/segment/coilgun/transfer(obj/structure/disposalholder/H)
	...
	for(var/obj/item/projectile/hvp/PJ in H.contents)
		if(PJ.velocity > safe_corner_velocity)
			take_damage(max(PJ.velocity - safe_corner_velocity, 10))
			if(QDELETED(src))
				return
	return ..()
