...
/obj/machinery/pool/drain/process()
	...
	if(active)
		if(filling)
			if(cycles_left-- > 0)
				playsound(src, 'austation/sound/effects/fillingwatter.ogg', 100, TRUE)
			...
			if(cycles_left-- > 0)
				playsound(src, 'austation/sound/effects/pooldrain.ogg', 100, TRUE)
				...
				for(var/mob/M in orange(min(drain_suck_range, 10), src))
					if(isliving(M) && !M.anchored && is_in_our_pool(M))
						...
						if(ishuman(M) && (get_dist(M, src) <= 1))
							...
							playsound(src, pick('austation/sound/misc/crack.ogg','austation/sound/misc/crunch.ogg'), 50, TRUE)
...
/obj/machinery/pool/filter/proc/spawn_shark()
	if(prob(50))
		...
		if(prob(50))
			...
			new /mob/living/simple_animal/hostile/retaliate/dolphin(loc)
