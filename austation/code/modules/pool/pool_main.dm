/turf/open/pool
	icon = 'austation/icons/turf/pool.dmi'
	name = "poolwater"
			...
			if(iscarbon(AM))		//FUN TIME!
				...
				if(filled)
					if (H.wear_mask && H.wear_mask.flags_cover & MASKCOVERSMOUTH)
						...
						playsound(src, 'austation/sound/effects/splash.ogg', 60, TRUE, 1)
						H.Knockdown(20)
						return
					else
						H.Knockdown(60)
					...
					if(prob(75))
						...
						playsound(src, 'sound/effects/woodhit.ogg', 60, TRUE, 1)
						playsound(src, 'austation/sound/misc/crack.ogg', 100, TRUE)
				else
...
/turf/open/pool/MouseDrop_T(atom/from, mob/user)
	...
	if(user.stat || !Adjacent(user) || !from.Adjacent(user) || !iscarbon(user) || !victim.has_gravity(src) || HAS_TRAIT(victim, TRAIT_SWIMMING))
		return
...
/turf/open/pool/attackby(obj/item/W, mob/living/user)
	if(istype(W, /obj/item/mop) && filled)
		...
		playsound(src, 'austation/sound/effects/slosh.ogg', 25, TRUE)
	else
		return ..()
...
/turf/open/pool/attack_hand(mob/living/user)
	...
	if((user.loc != src) && !user.IsStun() && !user.IsKnockdown() && !user.incapacitated() && Adjacent(user) && HAS_TRAIT(user, TRAIT_SWIMMING) && filled && (next_splash < world.time))
		playsound(src, 'austation/sound/effects/watersplash.ogg', 8, TRUE, 1)
