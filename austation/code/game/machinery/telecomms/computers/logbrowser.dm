...
/obj/machinery/computer/telecomms/server/ui_interact(mob/user)
	...
	// austation begin -- filtering menu
...
/obj/machinery/computer/telecomms/server/Topic(href, href_list)
	...
	if(href_list["operation"])
		switch(href_list["operation"])
			...
			// austation begin -- filter menu and back button operations
			if("filters")
				screen = 2
	...
	if(href_list["delete"])
		...
		if(SelectedServer)
			...
			// austation begin -- if on filter screen, delete filter instead of log
			if(screen == 2)
	...
	// austation begin -- add new filter
	if(href_list["newfilter"])
		...
		if(newfilter && (get_dist(usr, src) <= 1 || issilicon(usr)))
			...
			if(length(split_list) != 2)
				...
				// I know this is fucked but regex was being a bitch with brackets and honestly I don't care anymore because it works
