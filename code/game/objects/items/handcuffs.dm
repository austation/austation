...
/obj/item/restraints/handcuffs/attack(mob/living/carbon/C, mob/living/user)
	...
	if(!C.handcuffed)
		if(C.get_num_arms(FALSE) >= 2 || C.get_arm_ignore())
			...
			if(do_mob(user, C, (C.has_status_effect(STATUS_EFFECT_PARALYZED) ? 20 : 40)) && (C.get_num_arms(FALSE) >= 2 || C.get_arm_ignore())) //austation -- Makes handcuffing paralzyed people faster
				if(iscyborg(user))
