...
/datum/weather/rad_storm/weather_act(mob/living/L)
	...
	if(prob(40))
		if(ishuman(L))
			...
			if(H.dna && !HAS_TRAIT(H, TRAIT_RADIMMUNE) || HAS_TRAIT(H, TRAIT_MUTATEIMMUNE)) // austation -- don't give mutations to IPCs
				if(prob(max(0,100-resist)))
					...
					if(prob(50))
						...
						H.domutcheck()
				if(isipc(L)) // austation start -- IPC radiation interaction
					if(prob(max(0,100-resist)))
						if(prob(50))
